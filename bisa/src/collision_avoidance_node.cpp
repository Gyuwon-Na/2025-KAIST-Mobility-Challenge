 ...
 * @brief 충돌 방지 노드 v34.5
 *
 * 수정:
 * - HV 뒤쪽 끝점을 글로벌 패스 위에서 계산 (yaw 아님)
 * - HV 마커 제거 (gazebo 차량만 보이게)
 */
...
namespace bisa
{
    ...
    {
        // ==========================================
        // 1. HV IDs (Human Driven Vehicle) 파라미터 로드
        // ==========================================
        // 기본값으로 {19, 20}을 설정하지만, yaml에 있으면 덮어씌워짐
        ...
        // ==========================================
        // 2. CAV IDs (Connected Autonomous Vehicle) 파라미터 로드
        // ==========================================
    ...
    {
        control_timer_ = this->create_wall_timer(1ms, std::bind(&CollisionAvoidanceNode::control_loop, this));
        vis_timer_ = this->create_wall_timer(50ms, std::bind(&CollisionAvoidanceNode::publish_visualizations, this));
        debug_timer_ = this->create_wall_timer(500ms, std::bind(&CollisionAvoidanceNode::log_hv_debug, this));
    }
    ...
    void CollisionAvoidanceNode::hv_pose_callback(const geometry_msgs::msg::PoseStamped::SharedPtr msg, int hv_id)
    {
        ...
        if (!state.global_path.empty())
        {
            ...
            // ★★★ HV 뒤쪽 끝점: 글로벌 패스에서 뒤로 HV_REAR_OFFSET만큼 이동한 지점 ★★★
    ...
    // ★★★ 글로벌 패스에서 현재 위치로부터 뒤로 distance만큼 떨어진 점 찾기 ★★★
    ...
    void CollisionAvoidanceNode::update_hv_dynamic_path(int hv_id)
    {
        ...
        // ★★★ 1. 뒤쪽 끝점(rear_point)부터 현재 위치까지 경로 추가 ★★★
    ...
    std::tuple<bool, int, bool> CollisionAvoidanceNode::check_hv_collision(int cav_id)
    {
        ...
        const double TIP_ZONE_LENGTH = 0.35; // tip에서 뒤로 0.05m 구간

        // ★★★ tip에서 뒤로 0.05m 지점의 인덱스 찾기 ★★★
        ...
        for (int hv_id : hv_ids_)
        {
            ...
            // 1. CAV 전방 끝점과 HV 뒤쪽 끝점 거리
            double dist_to_hv_rear = cav_tip.dist_to(hv.rear_point);

            // 2. CAV 전방 끝점과 HV 중심 거리
            double dist_to_hv_center = cav_tip.dist_to(hv.position);

            // 3. HV 로컬패스가 CAV의 어디에 먼저 닿는지 확인
            //    - tip_zone: 끝점 ~ (끝점-0.05m) 구간 → 앞에서 오는 HV
            //    - behind_zone: (끝점-0.05m) ~ position 구간 → 뒤에서 오는 HV
            ...
            for (const auto &hp : hv.dynamic_path)
            {
                ...
                // behind zone (나머지 구간: position ~ tip-0.05m)
    ...
    std::tuple<bool, int, bool> CollisionAvoidanceNode::check_cav_collision(int cav_id)
    {
        ...
        for (int other_id : cav_ids_)
        {
            ...
            if (priority_cav == cav_id)
            {
                // 내가 우선순위 → GO (다음 CAV 확인)
                continue;
            }
            else
            {
                // ★★★ 수정: 겹치면 무조건 STOP (우선순위 낮으면) ★★★
    ...
    {
        auto &state = cav_states_[cav_id];
        double cur = state.current_velocity;
        double nv = (target > cur) ? std::min(cur + ACCEL_RATE, target) : std::max(cur - DECEL_RATE, target);
        state.current_velocity = nv;
        return nv;
    }
