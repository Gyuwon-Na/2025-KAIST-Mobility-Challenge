...
class HDMapVisualizer(Node):
    def __init__(self):
        ...
        # Publisher
        self.link_marker_pub = self.create_publisher(MarkerArray, '/hdmap/link_markers', qos)
        self.node_marker_pub = self.create_publisher(MarkerArray, '/hdmap/node_markers', qos)
        ...
        try:
            ...
            if not os.path.exists(load_path):
                ...
                load_path = os.path.join(current_dir, "../hdmap_data")
                
        except Exception as e:
            self.get_logger().error(f"Cannot find package path: {e}")
            current_dir = os.path.dirname(os.path.abspath(__file__))
            load_path = os.path.join(current_dir, "../hdmap_data")
    ...
    def create_link_markers(self):
        ...
        for i, link in enumerate(self.links.values()):
            marker = Marker()
            marker.header.frame_id = 'world'  # ← world 좌표계
