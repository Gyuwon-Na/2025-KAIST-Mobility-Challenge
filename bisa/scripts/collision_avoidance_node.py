...
class CollisionAvoidanceNode(Node):
    def __init__(self):
        ...
        for cav_id in self.cav_ids:
            self.create_subscription(
                Accel, f'/CAV_{cav_id:02d}_accel_raw',
        ...
        self.cav_marker_pub = self.create_publisher(MarkerArray, '/cav_markers', vis_qos)
        self.zone_marker_pub = self.create_publisher(MarkerArray, '/collision_zones', vis_qos)
            ...
            for t in [0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]:
                ...
                for i in range(entry_idx, min(entry_idx + 60, len(my_path))):
                    ...
                    if d < self.hv_danger_dist:
                        self.hv_debug_info[cav_id] = f"DANGER! HV{hv_id} t={t:.1f}s d={d:.2f}m path_idx={i}"
                        return True, hv_id
        ...
        for i in range(0, n, 4):
            ...
            for j in range(j_start, j_end, 3):
                ...
                if d < self.path_cross_dist:
                    # 방향 검사
                    if i >= 12 and j >= 12 and i < len(path1) - 3 and j < len(path2) - 3:
    ...
    def check_cav_collision(self, cav_id):
        ...
        for other_id in self.cav_ids:
            ...
            has_collision, zone, my_idx, other_idx = self.find_real_collision(my_path, other_path)
            ...
            other_in = self.dist(other_pos, zone) < self.collision_radius if zone else False
            ...
            my_dist = self.get_path_distance(my_path, self.find_closest_path_idx(my_pos, my_path), my_idx)
