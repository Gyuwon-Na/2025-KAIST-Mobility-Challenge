...
class HDMapGenerator:
    ...
    def scan_waypoint_files(self):
        ...
        if not os.path.exists(self.waypoint_dir):
            raise FileNotFoundError(f"Waypoint directory not found: {self.waypoint_dir}")
        
        csv_files = sorted([f for f in os.listdir(self.waypoint_dir) if f.endswith('.csv')])
        print(f"\n{'='*60}")
        ...
        for csv_file in csv_files:
            try:
                ...
                print(f"✓ {csv_file:20s} → {len(points):4d} points, {cost:6.2f}m")
            ...
            for row in rows[start_idx:]:
                try:
                    if len(row) >= 2:
                        ...
                        # 중복 체크 (0.1mm 단위)
                        if prev_x is None or abs(x - prev_x) > 0.0001 or abs(y - prev_y) > 0.0001:
