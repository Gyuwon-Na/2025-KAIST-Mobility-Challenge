...
class GlobalPathPublisherMulti(Node):
    def __init__(self):
        ...
        node_seq_param = self.get_parameter('node_sequence').get_parameter_value().integer_array_value
        self.node_sequence_input = list(node_seq_param)
        
        self.get_logger().info("="*60)
        self.get_logger().info(f"Global Path Publisher - CAV{self.cav_id:02d}")
    ...
    def calculate_global_path(self):
        try:
            ...
            self.get_logger().info(f"✓ CAV{self.cav_id:02d} Path Ready! {len(msg.poses)} points")
