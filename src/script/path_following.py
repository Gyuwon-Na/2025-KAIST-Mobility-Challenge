...
class MPCNode(Node):
    def __init__(self):
        ...
        if len(self.global_path) > 0:
            self.get_logger().info(
                f"MPC Node Ready. Total Waypoints: {len(self.global_path)}"
            )
    ...
    def publish_local_path(self, pred_path):
        if not pred_path or self.frame_id is None:
            return
        path_msg = Path()
        path_msg.header.frame_id = self.frame_id
        path_msg.header.stamp = self.get_clock().now().to_msg()
    ...
    def get_curvature(self, idx, lookahead_steps=20):
        if idx + lookahead_steps >= len(self.global_path):
            return 0.0
        curr_yaw = self.global_path[idx][2]
        max_diff = 0.0
        for i in range(1, lookahead_steps, 5):
            check_idx = idx + i
            if check_idx >= len(self.global_path):
                break
    ...
    def control_loop(self):
        ...
        if new_mode != self.current_mode:
            self.get_logger().info(
                f">>> [MODE] {self.current_mode} -> {new_mode} (Curv: {curvature:.4f})"
            )
